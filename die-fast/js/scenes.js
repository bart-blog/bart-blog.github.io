...
(function () {
  ...
  const S1 = {
    ...
    captions: [{ at: 1.6, to: 11.4, text: 'Your heart beats a hundred thousand times a day.' }],
    music(M) {
      M.pad(1, 'G3 D4 B4', 11, 0.4);
      M.sub(1, 'G2', 10, 0.26);
      LUB1.forEach((at, k) => { M.thump(at, 0.5 + k * 0.03); M.thump(at + 0.3, 0.3 + k * 0.02); });
      M.ep(1, 'B5', 0.45); M.ep(3, 'D6', 0.14);
      M.ep(5, 'G5', 0.42); M.ep(7, 'D6', 0.14);
      M.ep(9, 'A5', 0.42); M.ep(10, 'B5', 0.14); M.ep(11, 'G6', 0.14);
    },
  };
  ...
  const S2 = {
    ...
    captions: [
      { at: 1.2, to: 9.6, text: 'Most days, you feel perfectly fine.' },
      { at: 10.2, to: 19.5, text: 'But feeling fine is only half the story.' },
    ],
    music(M) {
      const CH = [['G3 B3 D4', 'G2'], ['E3 G3 B3', 'E2'], ['C3 G3 E4', 'C2'], ['D3 A3 F#4', 'D2'], ['G3 B3 D4 A4', 'G2']];
      CH.forEach(([p, s], k) => { M.pad(k * 4, p, 4, 0.3); M.sub(k * 4, s, 4, 0.2); });
      R2.forEach((at, k) => { M.beep(at, 'B5', 0.16 + (k < 5 ? 0 : -0.03), 0.35); M.thump(at, 0.32); M.thump(at + 0.3, 0.18); });
      M.ep(2, 'D5', 0.16); M.ep(4.5, 'G5', 0.14); M.ep(6, 'B5', 0.14); M.ep(8.5, 'A5', 0.14);
      M.ep(10.2, 'E5', 0.16); M.ep(12, 'G5', 0.12); M.ep(14, 'F#5', 0.12); M.ep(16, 'D5', 0.12); M.ep(18, 'B4', 0.12);
    },
  };
  ...
  const WALLN = 44, CELLN = 50, PLAQN = 11, WORD3 = [1.2, 3.4, 5.6], PL3 = 10.4, H3 = odd(1, 20);
  ...
  const flow3 = perBeat((b) => {
    ...
    const h = 1 / 60;
    for (let x = 0; x < b; x += h) {
      const P = E.inOutSine(clamp((x - PL3) / 7));
  ...
  const S3 = {
    ...
    blend: {
      ...
      start: (r) => (r >= CELL0 && r < PLAQ0 ? 0.1 + ((r - CELL0) / CELLN) * 0.6 : 0),
      dur: (r) => (r >= PLAQ0 ? 0.01 : 1.1),
    },
    backdrop: (b) => {
      const hv = hv3() * (1 + 0.05 * pulse3(b));
      return { a: E.inOutSine(clamp(b / 1.4)) * (1 - E.inOutSine(clamp((b - 18.6) / 1.4))), top: -hv, h: 2 * hv };
    },
    pose(r, t, b, o) {
      const S = span3();
      if (r === 0) {
      ...
      if (r < PLAQ0) {
        ...
        const f = clamp(2.2 * hit(b, WORD3[c.g] + 0.2 + (r - CELL0) * 0.004, 1.3));
    ...
    captions: [
      { at: 1.2, to: 9.6, words: [['Cholesterol.', 1.2], ['Blood sugar.', 3.4], ['Inflammation.', 5.6]] },
      { at: 10.2, to: 19.5, text: 'They can change silently, for years.' },
    ],
    music(M) {
      M.pad(0, 'E3 B3 G4', 5, 0.3); M.sub(0, 'E2', 5, 0.22);
      M.pad(5, 'C3 G3 E4', 5, 0.28); M.sub(5, 'C2', 5, 0.2);
      ...
      M.whoosh(0, 1.4, 0.05, 300, 1400);
      M.pad(10, 'A2 E3 C4', 4, 0.26); M.sub(10, 'A1', 4, 0.2);
      M.pad(14, 'F2 C3 A3', 3, 0.24); M.sub(14, 'F1', 3, 0.2);
      M.pad(17, 'B2 F#3 D#4', 3, 0.24); M.sub(17, 'B1', 3, 0.2);
      const PL = ['B4', 'A4', 'G4', 'F#4', 'E4', 'D4', 'E4', 'F#4', 'D#4', 'B3', 'B3'];
      for (let k = 0; k < PLAQN; k++) M.marimba(plaqueAt(k) + 0.1, PL[k], 0.12, 0.3);
    },
  };
  ...
  const QN = 104, FR0 = 1 + QN, LF0 = FR0 + 24, ADM4 = [1.5, 5.5, 9.5, 13.5];
  const Q4 = perLayout(() => {
  ...
  const adv4 = (b, s0) => {
    ...
    for (let k = 0; k < ADM4.length; k++) {
      const u = clamp((b - ADM4[k] - 0.012 * Math.max(0, s0 - k)) / 0.7);
  ...
  function queuePose(r, b, o) {
    ...
    if (s >= 0) {
      ...
      if (r === 0) { const w = maxHit(b, [11, 11.5, 15], 5); o.y -= w * G.dot * 0.9; }
      return;
    }
    const u = clamp(-s);
  ...
  const S4 = {
    ...
    blend: {
      ...
      start: (r) => (r === 0 ? 0.6 : r < FR0 ? 0.1 + (r / QN) * 0.9 : 0.2 + ((r - FR0) / 45) * 0.6),
      dur: (r) => (r === 0 ? 1.6 : 1.2),
    },
    pose(r, t, b, o) { queuePose(r, b, o); },
    captions: [
      { at: 1.4, to: 8.6, text: 'Normally, you’d need a referral from your GP first.' },
      { at: 9.4, to: 15.5, text: 'And then, you wait.' },
    ],
    music(M) {
      M.pad(0, 'E3 B3 D4 G4', 4, 0.24); M.sub(0, 'E2', 4, 0.18);
      M.pad(4, 'C3 G3 B3 E4', 4, 0.22); M.sub(4, 'C2', 4, 0.16);
      M.pad(8, 'A2 E3 G3 C4', 4, 0.22); M.sub(8, 'A1', 4, 0.16);
      M.pad(12, 'B2 F#3 A3 D4', 4, 0.22); M.sub(12, 'B1', 4, 0.16);
      for (let b = 1; b < 16; b++) M.tick(b, b % 2 ? 0.05 : 0.03, 0.5);
      ADM4.forEach((at, k) => { M.snip(at - 0.1); M.marimba(at + 0.3, ['E4', 'D4', 'C4', 'B3'][k], 0.16, 0.6); });
      M.ep(2.5, 'G5', 0.1); M.ep(6.5, 'E5', 0.1); M.ep(10.5, 'C5', 0.1); M.ep(14.5, 'B4', 0.1);
      [11, 11.5, 15].forEach((at) => M.drop(at, 'B5', 0.08, -0.5));
    },
  };
  ...
  const OPEN5 = 0.4, HOP5 = [4.4, 5.1, 5.8], HOPD = 0.7, DASH5 = HOP5[2] + HOPD;
  ...
  const S5 = {
    ...
    pose(r, t, b, o) {
      ...
      queuePose(r, 16, o);
      if (r >= LF0) {
        const th = 1.45 * E.spring(clamp((b - OPEN5) / 1.4));
        leafPose(r - LF0, th, o);
        o.c = 0.9 * E.outCubic(clamp((b - OPEN5) / 1.2));
        o.o = lerp(0.5, 0.85, o.c) * (1 - E.inOutSine(clamp((b - 9.2 - (r - LF0) * 0.03) / 1.2)));
        return;
      }
      if (r >= FR0) {
        o.o = 1 - E.inOutSine(clamp((b - 9 - (r - FR0) * 0.03) / 1.2));
        return;
      }
      const h = hop5(b);
      if (r === 0) {
        ...
        const s = lerp(1.15, 1.5, E.outCubic(clamp((b - DASH5) / 1)));
      ...
      const f = E.inOutCubic(clamp((b - 7.4 - k * 1.6) / 1.4));
      o.x = lerp(o.x, L.dc[0], f * 0.35); o.y = lerp(o.y, L.dc[1] + L.fs * 3, f * 0.35);
      o.o *= 1 - f;
    },
    captions: [
      { at: 0.6, to: 5.8, text: 'With LiveLong, you don’t need one.' },
      { at: DASH5, to: 11.5, words: [['No referral.', DASH5], ['No waiting list.', 7.6]] },
    ],
    music(M) {
      M.pad(0, 'G3 B3 D4 A4', DASH5, 0.26); M.sub(0, 'G2', DASH5, 0.2);
      M.whoosh(OPEN5, 1.4, 0.08, 400, 2200);
      M.ep(OPEN5 + 0.3, 'D5', 0.2); M.ep(OPEN5 + 0.6, 'G5', 0.2); M.ep(OPEN5 + 0.9, 'B5', 0.2);
      ...
      M.pad(DASH5, 'C4 E4 G4 B4 D5', 5.5, 0.3); M.sub(DASH5, 'C2', 5.5, 0.26);
      M.ep(7.6, 'E6', 0.18); M.ep(7.62, 'G5', 0.14);
      M.ep(9, 'D6', 0.12); M.ep(10, 'B5', 0.12);
    },
  };
  ...
  const NLP = SH.nl.pts, AMS = SH.nl.ams, LAND6 = 2.8;
  ...
  const LIT6 = (function () {
    ...
    for (let k = 0; k < 108; k++) { const i = idx[k], p = NLP[i]; lit[i] = 4.2 + (Math.hypot(p[0] - AMS[0], p[1] - AMS[1]) / dmax) * 4.5; }
    return lit;
  })();
  const H6 = odd(11, 16);
  const S6 = {
    ...
    blend: {
      arc: 0.1,
      start: (r) => (r === 0 ? 0.4 : 0.1 + (NLP[r - 1][0] + 0.5) * 1.6),
      dur: (r) => (r === 0 ? LAND6 - 0.4 : 1.1),
      ease: E.glide,
    },
    pose(r, t, b, o) {
      const Wm = mapW();
    ...
    captions: [
      { at: 3.4, to: 9.8, text: 'Walk in at one of 108 locations.' },
      { at: 10.4, to: 15.5, text: 'All across the Netherlands.' },
    ],
    music(M) {
      M.pad(0, 'G3 D4 B4', 4, 0.26); M.sub(0, 'G2', 4, 0.2);
      M.whoosh(0.4, 2.4, 0.08, 300, 1800);
      M.drop(LAND6, 'G5', 0.24); M.bell(LAND6, 'D6', 0.2); M.kick(LAND6, 0.3);
      M.pad(4, 'E3 B3 D4 G4', 4, 0.26); M.sub(4, 'E2', 4, 0.2);
      M.pad(8, 'C3 G3 B3 E4', 4, 0.26); M.sub(8, 'C2', 4, 0.2);
      M.pad(12, 'D3 A3 C4 F#4', 4, 0.26); M.sub(12, 'D2', 4, 0.2);
      const AR = ['G5', 'A5', 'B5', 'D6', 'E6', 'G6', 'E6', 'D6', 'B5', 'A5', 'B5', 'D6', 'E6', 'D6', 'B5', 'G5', 'A5', 'B5'];
      AR.forEach((n, k) => M.marimba(4.2 + k * 0.25, n, 0.14 - k * 0.003, ((k % 5) / 4 - 0.5) * 0.9));
      H6.forEach((at) => { M.thump(at, 0.28); M.thump(at + 0.3, 0.16); });
      M.ep(10.4, 'B5', 0.16); M.ep(11.4, 'A5', 0.12); M.ep(12.4, 'G5', 0.12); M.ep(14.4, 'F#5', 0.12);
    },
  };
  ...
  const FILL = 80, OUT0 = 1 + FILL, CAP0 = OUT0 + 56, SPAWN7 = 1.2, DT7 = 0.055, CAP7 = 8.6;
  ...
  const S7 = {
    ...
    blend: {
      ...
      start: (r) => (r <= FILL ? spawn7(r) - 0.75 : 0.1 + ((r - OUT0) / 66) * 0.8),
      dur: (r) => (r <= FILL ? 0.7 : 1.1),
    },
    camera: (b) => {
      const u = clamp((b - 9.4) / 2);
    ...
    update(t, b, dt) {
      ...
      for (let r = 0; r <= FILL; r++) {
        ...
        w.vx[i] = Math.sin(i * 78.233) * G.R * 0.05; w.vy[i] = G.R * 0.8;
      }
      w.step(dt, {
    ...
    captions: [{ at: 1.6, to: 11.5, text: 'One quick blood draw.' }],
    music(M) {
      M.pad(0, 'G3 B3 D4 F#4', 4, 0.24); M.sub(0, 'G2', 4, 0.18);
      M.pad(4, 'E3 G3 B3 D4', 4, 0.24); M.sub(4, 'E2', 4, 0.18);
      M.pad(8, 'C3 G3 B3 E4', 4, 0.26); M.sub(8, 'C2', 4, 0.2);
      ...
      M.whoosh(9.4, 1, 0.04, 400, 900); M.whoosh(10.4, 1, 0.04, 900, 400);
    },
  };
  ...
  const HUB0 = 1, TUB0 = 30, WM8 = (60 * (TAU / 6)) / 6.3;
  function phi8(b) {
    if (b < 1.4) return 0;
    if (b < 5) { const u = (b - 1.4) / 3.6; return WM8 * 3.6 * (u ** 3 - u ** 4 / 2); }
    if (b < 8) return WM8 * (1.8 + (b - 5));
    if (b < 11) { const v = (b - 8) / 3; return WM8 * (4.8 + 3 * (v - (v ** 3 - v ** 4 / 2))); }
    return WM8 * 6.3;
  }
  const tilt8 = (b) => 0.95 * E.inOutSine(clamp((b - 0.3) / 1.3)) * (1 - E.inOutSine(clamp((b - 10.5) / 1.4)));
  const S8 = {
    name: 'centrifuge',
    beats: 12,
    blend: { ease: E.inOutCubic, arc: 0.1, start: (r) => (r === 0 ? 0 : (r / N) * 0.5), dur: () => 1.2 },
    pose(r, t, b, o) {
      const Rr = G.R * 0.95, ph = phi8(b), tau = tilt8(b);
      let x, y;
      if (r === 0) {
        x = y = 0;
        o.sx = o.sy = 1.4 * (1 + 0.2 * hit(b, 11, 3));
        o.c = 1;
      } else if (r < TUB0) {
        ...
        const sep = E.inOutCubic(clamp((b - 4.5 - (9 - row) * 0.12) / 1.5));
    ...
    captions: [
      { at: 0.8, to: 6.6, text: 'Analysed in a medical lab.' },
      { at: 7.2, to: 11.6, text: 'ISO 15189 accredited.' },
    ],
    music(M) {
      M.pad(0, 'A2 E3 G3 C4', 4, 0.24); M.sub(0, 'A1', 4, 0.2);
      M.pad(4, 'D3 A3 C4 F#4', 4, 0.26); M.sub(4, 'D2', 4, 0.22);
      M.pad(8, 'G3 B3 D4 A4', 4, 0.28); M.sub(8, 'G2', 4, 0.22);
      M.whoosh(1.4, 3.6, 0.1, 150, 1800); M.whoosh(5, 3, 0.05, 1800, 1800); M.whoosh(8, 3, 0.08, 1800, 150);
      let last = 0;
      for (let b = 1.4; b < 11.2; b += 0.01) { const rev = Math.floor(phi8(b) / TAU); if (rev > last) { last = rev; M.tick(b, 0.05, 0); } }
      [4.6, 4.9, 5.2, 5.5].forEach((at, k) => M.marimba(at, ['B5', 'G5', 'E5', 'D5'][k], 0.12, 0));
      M.bell(7.2, 'G6', 0.16); M.ep(7.2, 'D5', 0.2); M.ep(7.5, 'G5', 0.16);
      M.drop(11, 'G5', 0.14);
    },
  };
  ...
  const RUN9 = [1.4, 9.4], DONE9 = 9.5, TXT0 = 49;
  ...
  const S9 = {
    ...
    blend: {
      ease: E.inOutCubic, arc: 0.12,
      start: (r) => (r === 0 ? 0 : r < TXT0 ? 0.1 + (r / 48) * 0.5 : 0.3 + (SH.h48.pts[r - TXT0][0] + 0.5) * 0.8),
      dur: () => 1.1,
    },
    pose(r, t, b, o) {
    ...
    captions: [{ at: 1.2, to: 11.5, text: 'Results within 48 hours.' }],
    music(M) {
      M.pad(0, 'C3 G3 B3 E4', 4, 0.24); M.sub(0, 'C2', 4, 0.18);
      M.pad(4, 'A2 E3 G3 C4', 3, 0.24); M.sub(4, 'A1', 3, 0.18);
      M.pad(7, 'D3 A3 C4 F#4', 2.5, 0.26); M.sub(7, 'D2', 2.5, 0.2);
      LIT9.forEach((at, k) => M.tick(at, k % 12 === 0 ? 0.09 : 0.04, Math.sin((k / 48) * TAU) * 0.7));
      ...
      M.pad(DONE9, 'G3 D4 B4 F#5', 2.5, 0.3); M.sub(DONE9, 'G2', 2.5, 0.26);
    },
  };
  ...
  const VALS = [0.46, 0.55, 0.78, 0.4, 0.62, 0.84, 0.5], ZONE = [0.25, 0.7], MK10 = (i) => 2.4 + i * 0.3, RV10 = (i) => 6.4 + i * 0.55;
  const L10 = perLayout(() => {
  ...
  const S10 = {
    name: 'results',
    beats: 16,
    blend: { ease: E.glide, arc: 0.1, start: (r) => (r === 0 ? 0.2 : 0.1 + (Math.floor((r - 1) / 21) / 7) * 1.2 + (((r - 1) % 21) / 21) * 0.3), dur: () => 1.1 },
    pose(r, t, b, o) {
      const L = L10();
      if (r === 0) {
        let y = rowY10(0) - L.rs * 0.9, sq = 0;
        for (let i = 0; i < 7; i++) {
          const T = RV10(i), p = E.inOutCubic(clamp((b - T + 0.45) / 0.4));
      ...
      if (j < 3) {
        ...
        o.o = 0.5 + 0.5 * chk; o.c = warn ? E.outCubic(clamp((b - 11) / 0.6)) : 0;
        return;
      }
      if (j < 20) {
      ...
      const T = MK10(i), p = E.spring(clamp((b - T) / 1.1)), land = hit(b, T + 0.25, 4);
      o.x = L.xT + VALS[i] * p * L.Wt; o.y = y;
      o.sx = o.sy = 1.15 * (1 + 0.3 * land + (warn ? 0.2 * maxHit(b, [11, 13, 15], 3) : 0));
      o.o = clamp((b - T + 0.3) / 0.3); o.c = warn ? clamp((b - T - 0.5) / 0.5) : 0;
    },
    captions: [
      { at: 1.2, to: 5.8, text: '43 biomarkers across 10 health areas.' },
      { at: 6.4, to: 10.4, text: 'Every result reviewed by a doctor.' },
      { at: 11, to: 15.5, text: 'With clear advice for every value.' },
    ],
    music(M) {
      M.pad(0, 'G3 B3 D4 A4', 4, 0.24); M.sub(0, 'G2', 4, 0.18);
      M.pad(4, 'E3 G3 B3 D4', 4, 0.24); M.sub(4, 'E2', 4, 0.18);
      M.pad(8, 'C3 G3 B3 E4', 4, 0.24); M.sub(8, 'C2', 4, 0.18);
      M.pad(12, 'D3 A3 C4 F#4', 4, 0.26); M.sub(12, 'D2', 4, 0.2);
      VALS.forEach((v, i) => M.marimba(MK10(i) + 0.25, ['G4', 'A4', 'B4', 'D5', 'E5', 'G5', 'A5'][Math.min(6, Math.floor(v * 8))], 0.16, v - 0.5));
      for (let i = 0; i < 7; i++) { M.tick(RV10(i), 0.08, 0.6); M.ep(RV10(i), ['D6', 'B5', 'G5', 'A5', 'B5', 'D6', 'G6'][i], 0.1, 0.5); }
      [11, 13, 15].forEach((at) => { M.bell(at, 'E6', 0.1, -0.2); M.ep(at, 'B5', 0.12); });
    },
  };

  // =========================================================== 11 · over time
  // A trend line, drawn by the orange dot from one test to the next.
  const VAL11 = [0.28, 0.46, 0.4, 0.64, 0.82], T11 = (i) => 1.4 + i * 2.2, LN0 = 6, AX0 = LN0 + 114;
  const L11 = perLayout(() => {
  ...
  const penX11 = (b) => {
    ...
    for (let i = 0; i < 4; i++) if (b < T11(i + 1)) return lerp(L.cx[i], L.cx[i + 1], E.inOutSine((b - T11(i)) / 2.2));
    return L.cx[4];
  };
  const S11 = {
    name: 'trend',
    beats: 16,
    blend: { ease: E.glide, arc: 0.12, start: (r) => (r === 0 ? 0.1 : r < AX0 ? 0.3 + (r / AX0) * 0.4 : 0.2 + ((r - AX0) / 30) * 0.6), dur: () => 1.1 },
    pose(r, t, b, o) {
      const L = L11(), px = penX11(b);
      if (r === 0) {
        o.x = px; o.y = L.yAt(px);
        const p = maxHit(b, [0, 1, 2, 3, 4].map(T11), 4) + maxHit(b, [11, 13, 15], 4) * 0.8;
    ...
    captions: [
      { at: 1.2, to: 7.6, text: 'Retest every 6 to 12 months.' },
      { at: 8.2, to: 15.5, text: 'And see your progress over time.' },
    ],
    music(M) {
      const CH = [['G3 B3 D4', 'G2'], ['E3 G3 B3', 'E2'], ['C3 E3 G3 B3', 'C2'], ['D3 F#3 A3', 'D2'], ['G3 B3 D4 F#4', 'G2']];
      CH.forEach(([p, s], i) => { const at = i ? T11(i) : 0; const nx = i < 4 ? T11(i + 1) : 16; M.pad(at, p, nx - at, 0.26); M.sub(at, s, nx - at, 0.2); });
      ['D5', 'G5', 'E5', 'B5', 'D6'].forEach((n, i) => { M.bell(T11(i), n, 0.18, (i / 4 - 0.5) * 0.8); M.ep(T11(i), n, 0.18); });
      for (let i = 0; i < 4; i++) M.whoosh(T11(i) + 0.2, 1.8, 0.03, 600, 1200 + i * 300);
      [11, 13, 15].forEach((at) => { M.thump(at, 0.24); M.thump(at + 0.3, 0.14); });
      M.ep(12, 'B5', 0.12); M.ep(13, 'A5', 0.12); M.ep(14, 'G5', 0.14);
    },
  };
  ...
  const LAND12 = (L) => 1.6 + L * 0.42, DROP12 = 5.2, BEAT12 = odd(7, 20);
  const markAt = LG.mark.map(([x, y]) => 0.2 + frac((Math.atan2(y - MC[1], x - MC[0]) + Math.PI / 2) / TAU + 1) * 1.6);
  ...
  const S12 = {
    ...
    blend: {
      ...
      start: (r) => (r === 0 ? 0 : r < BR0 ? Math.max(0, markAt[r - MK0] - 0.5) : r < WD0 ? 1.6 : LAND12(letter12(LG.word[r - WD0][0])) - 0.9),
      dur: (r) => (r === 0 ? 1.4 : r < BR0 ? 0.5 : r < WD0 ? 0.8 : 0.9),
    },
    pose(r, t, b, o) {
    ...
    captions: [
      { at: 8.4, to: Infinity, text: 'Take charge of your health.' },
      {
        at: 10.4, to: Infinity, cls: 'link',
    ...
    music(M) {
      M.pad(0, 'C3 G3 B3 E4', 2.8, 0.24); M.sub(0, 'C2', 2.8, 0.18);
      M.whoosh(0.2, 1.8, 0.07, 300, 2600);
      for (let k = 0; k < 8; k++) M.tick(0.2 + k * 0.2, 0.04, (k / 7 - 0.5) * 0.8);
      M.snip(2.4); M.snip(2.5);
      ['G5', 'A5', 'B5', 'D6', 'E6', 'G6', 'A6', 'B6'].forEach((n, L) => M.marimba(LAND12(L), n, 0.2 - L * 0.01, (L / 7 - 0.5) * 0.9));
      M.pad(2.8, 'D3 A3 C4 F#4', 2.6, 0.24); M.sub(2.8, 'D2', 2.6, 0.2);
      M.drop(DROP12 + 0.2, 'D6', 0.28); M.bell(DROP12 + 0.2, 'G6', 0.22);
      BEAT12.forEach((at, k) => { M.thump(at, 0.5 - k * 0.03); M.thump(at + 0.3, 0.3 - k * 0.02); });
      M.boom(7, 0.45); M.kick(7, 0.36); M.bell(7, 'D7', 0.14);
      M.ep(7, 'G3', 0.36); M.ep(7, 'G2', 0.3); M.ep(7.03, 'B5', 0.2); M.ep(7.06, 'D6', 0.18);
      M.pad(7, 'G3 D4 F#4 A4 B4', 5, 0.3); M.sub(7, 'G1', 5, 0.28);
      M.ep(8.4, 'B5', 0.18); M.ep(8.9, 'D6', 0.14); M.ep(9.4, 'A5', 0.14);
      M.ep(10.4, 'G5', 0.2); M.bell(10.4, 'G6', 0.1);
      M.pad(12, 'C3 G3 B3 E4', 4, 0.22); M.sub(12, 'C2', 4, 0.18);
      M.pad(16, 'G3 D4 B4', 4, 0.22); M.sub(16, 'G1', 4, 0.2);
      M.ep(16, 'G5', 0.14); M.ep(17, 'B5', 0.1); M.ep(19, 'D6', 0.08);
    },
  };
