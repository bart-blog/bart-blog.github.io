...
(function () {
  ...
  // ---------------------------------------------------------------- renderer
  // Dots are drawn large and scaled down so big formations (tiles, the sun) stay crisp.
  // Dot 0 is the hero — the yellow one — and gets an even larger canvas.
  const DOT_PX = 128, HERO_PX = 384;
  // c: 0 = Risqui navy, 1 = Risqui yellow. Mid-way it brightens a little so a
  // change of colour reads as a flash of light rather than a muddy blend.
  const NAVY = [0, 23, 82], GOLD = [252, 186, 4];
  function colour(c) {
  ...
  function Dots(stage) {
    ...
    this.sm = new Float32Array(N).fill(1); // how much a dot may smear (1 = fully)
    this.k.fill(1);
    this.ux.fill(1);
    this.lo.fill(-1);
    this.lc.fill(-1);
    ...
    for (let i = 0; i < N; i++) {
      ...
      if (dt > 0.004 && !this.fresh) {
        ...
        if (sp * dt > G.R * 0.5) {
          ...
          const target = 1 + this.sm[i] * Math.min(Math.max(0, sp - G.R * 0.8) / (G.R * 8), 1.3);
