...
(function () {
  ...
  const K = ['x', 'y', 'z', 'w', 'h', 'r', 'tip', 'rot', 'rx', 'ry', 'tl', 'bw', 'o', 'cr', 'cg', 'cb'];
  ...
  function render() {
    for (let i = 0; i < N; i++) {
      ...
      const bw = S.bw[i] > 0.25 ? Math.round(S.bw[i] * 4) / 4 : 0, bk = ck + bw;
      if (bk !== lc[i]) { // bw > 0: an outlined ring instead of a filled shape
        el.style.backgroundColor = bw ? 'transparent' : ck;
        el.style.border = bw ? bw + 'px solid ' + ck : '0';
        lc[i] = bk;
      }
      ...
      let a00 = cz * cy, a01 = cz * sy * sx - sz * cx, a02 = cz * sy * cx + sz * sx;
      let a10 = sz * cy, a11 = sz * sy * sx + cz * cx, a12 = sz * sy * cx - cz * sx;
      let a20 = -sy, a21 = cy * sx, a22 = cy * cx;
      if (S.tl[i]) { // tilt: an extra Rx applied after everything, so flat artwork can lie down in 3D
        const ct = Math.cos(S.tl[i]), st = Math.sin(S.tl[i]);
        const b10 = ct * a10 - st * a20, b11 = ct * a11 - st * a21, b12 = ct * a12 - st * a22;
        a20 = st * a10 + ct * a20; a21 = st * a11 + ct * a21; a22 = st * a12 + ct * a22;
  ...
  const q = {}, fq = {};
  function reset(p) {
    p.x = p.y = p.z = p.w = p.h = p.r = p.tip = p.rot = p.rx = p.ry = p.tl = p.bw = p.sym = 0;
    p.o = 1; p.c = C.azure;
  }
    ...
    for (let i = 0; i < N; i++) {
      ...
      if (raw < 1) {
        ...
        if (fv || tv) {
          ...
          for (const k of ['z', 'rot', 'rx', 'ry', 'tl']) q[k] = lerp(fq[k], q[k], p);
          for (const k of ['w', 'h', 'r', 'bw']) q[k] = Math.max(0, lerp(fq[k], q[k], p));
          for (const k of ['tip', 'o', 'cr', 'cg', 'cb']) q[k] = lerp(fq[k], q[k], pc);
        }
      }
      store(S, i, q);
    }
    setBg(sc.bg ? sc.bg(b) : C.bg);
  }
  let lastBg = '';
  function setBg(c) {
    const s = BJ.css(c);
  ...
  function acquire(it) {
    ...
    if (it.html) el.innerHTML = it.html;
      ...
      parts.forEach((w, k) => {
        ...
        s.className = w[0] === '*' ? 'w g' : 'w'; // *word: brand gradient, like the site's headlines
        s.textContent = w[0] === '*' ? w.slice(1) : w;
        el.appendChild(s);
  ...
  function styleLine(it) {
    ...
    st.left = it.x ? (G.cx + it.x()).toFixed(1) + 'px' : '';
    st.color = it.color || '';
  ...
  function introFrame(now) {
    ...
    for (let i = 0; i < N; i++) {
      ...
      if (i === (sc.introPulse || 0)) { const s = 1 + 0.05 * Math.sin(now * 2.2); q.w *= s; q.h *= s; q.r *= s; }
      store(S, i, q);
    }
    setBg(C.bg);
  }
