...
(function () {
  ...
  function World(n) {
    ...
    this.live = new Uint8Array(n).fill(1); // bodies that haven't been spawned yet are skipped
    this.active = false;
  }

  World.prototype.step = function (dt, opt) {
    const n = this.n, x = this.x, y = this.y, vx = this.vx, vy = this.vy, r = this.r, cool = this.cool, live = this.live;
